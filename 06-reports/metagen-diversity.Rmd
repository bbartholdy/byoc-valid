...
PC1 sees mostly negative loadings, especially aerobes and facultative anaerobes.
PC2 is slightly more balanced.
...
PC3 separates most artificial calculus from medium samples.
...
pca_loadings %>%
  ...
  ggplot(aes(x = PC3, y = species, fill = `Oxygen tolerance`)) +
    geom_bar(stat = "identity", width = 0.8) +
    scale_x_continuous(limits = c(-0.21,0.21)) +
    scale_fill_manual(
      values = viridisLite::turbo(
        n = length(levels(as.factor(bac_properties$`Oxygen tolerance`)))
        )
...
Time-lag plot of Bray-Curtis data
