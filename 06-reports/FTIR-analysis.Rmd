...
knitr::kable(n_samples, digits = 2, caption = "Summary of FTIR samples used in analysis, including weight in mg. Only a portion of the samples were analysed, so the weight is not indicative of the amount of calculus analysed.")
```
...
ftir_metadata %>%
  filter(!is.na(ftir_metadata$comb)) %>%
  #group_by(day) #%>%
  select(sample_id, day, comb) %>%
  mutate(comb = str_remove(comb, "_grind_a")) %>%
  distinct(comb, .keep_all = T)
```
