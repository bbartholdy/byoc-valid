...
Run SourceTracker2 on the filtered OTU table with rarefaction depth of 10000
...
sourcetracker2_long %>% 
  ggplot(aes(SampleID, proportion, fill = source)) +
    geom_col() +
    theme_minimal() +
    scale_x_discrete(limits = day_order$`#SampleID`) +
    scale_fill_viridis_d() +
    theme(axis.title = element_blank(),
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.5, colour = day_order$col))
```

Figure \@ref(fig:st-plot) shows that many of the later medium samples were assigned
to the 'Unknown' and 'indoor_air' categories. To see whether this was the result of external
...
all_data_long %>% 
  ...
  ggplot(aes(x = SampleID, y = count, fill = oral_source)) +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, colour = day_order$col),
          axis.title.x = element_blank()) +
    scale_x_discrete(limits = day_order$`#SampleID`)
```

Figure \@ref(fig:unknowns-plot) shows that a large proportion of the species
assigned to the various sources are known to be oral taxa. The samples do also
seem to contain some external contamination.
