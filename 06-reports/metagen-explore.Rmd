...
The otu table with contaminants removed is convert to long format and a column for
...
top_20_species <- species_abund_long %>%
  filter(str_detect(sample, "SYN")) %>% 
  ungroup() %>%
  arrange(desc(rel_abund)) %>%
  distinct(species) %>%
...
Most notable difference is the absence of aerobic species in the artificial
...
No aerobes made it into the model. Apart from the donated saliva, no aerobes
were detected in any of the experimental samples. ABS are also underrepresented
