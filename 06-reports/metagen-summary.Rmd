...
```{r}
# trend of top diff abund species over duration of experiment
  # Select species with highest 'drop-off' btw medium and calculus
...
clr_byoc_long %>%
  right_join(experiment_metadata, by = c("sample" = "#SampleID")) %>%
  filter(species %in% top_da_species) %>%
  ggplot(aes(x = day, y = clr_count, col = species)) +
    geom_jitter(width = 0.2) +
    facet_wrap(~ species)

# seems to be a gradual decline rather than a sharp dropoff, supporting that
  # differences btw medium and calculus are related to maturation stage of biofilm (Velsko et al. 2019)

# trend with bacteria with highest increase in model calculus

top_da_species2 <- byoc_logf_full %>%
  filter(abn == "byoc_calculus" |
           str_detect(species, "Enterococcus")) %>%
  arrange(desc(lfc)) %>%
  distinct(species) %>%
  slice_head(n = 15) %>%
  .$species
...
<!-- which species are there no differences between samples? -->
<!-- how many species are different vs. not different between model and real? -->
