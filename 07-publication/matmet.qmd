...
[@bartholdyInvestigatingBiases2021].
...
Samples were taken from the artificial saliva during the experiment, and from the
...
Processing of the raw DNA reads was conducted using the EAGER pipeline v. 2.4.1 [@yatesEAGER2020].
...
Reference samples downloaded from... Singletons were removed.
...
<!-- FTIR specs -->
FTIR analysis was conducted on `r nrow(byoc_ftir)` samples of the biofilm sampled
on days 7, 12, 16, 20, and 24. Samples
...
(`r paste(unique(subset(ftir_metadata, !is.na(ftir_metadata$comb))$comb))`).
Samples were ground together with KBr and pressed under up to two tons of pressure.
...
Grinding curves were prepared for samples from days 16, 20, and 24. We were unable
to obtain grinding curves from days 7 and 12 due to insufficient mineralisation.
Samples were ground and analysed up to six times (suffix grind_[a-f]) for the grind curve,
with samples denoted 'a' ground for the least amount of time and 'f' the most.
...
between wavenumbers 4000 and 400 cm$^{-1}$.
The splitting factor was calculated following -@asscherAtomicDisorder2011,
using the full width at half maximum (FWHM) of the main peak at 1040.
