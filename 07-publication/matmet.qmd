...
conditions, we sampled biofilms on multiple days and determined the mineral
composition using FTIR, and compared the spectra to those of natural
...
AS was partially refreshed once per day, by topping up the wells to the original
volume with more AS, and fully refreshed every three days,
...
counts being
affected by $\alpha$-amylase hydrolysis from the inoculation saliva. An
$\alpha$-amylase
assay conducted on samples from days 3, 6, 8, 9, 10, 12, and 14 confirmed the
absence of $\alpha$-amylase activity in the system.
...
samples using the **vegan** `r packageVersion("vegan")` R package [@Rvegan]. Shannon index
...
reference samples. The sPCA analysis was conducted using the **mixOmics** R package
`r packageVersion("mixOmics")` [@RmixOmics].  
...
**ANCOMBC** R package [@linANCOMBC2020], with species-level OTU as input. Results
...
plaque, modern calculus, model calculus).
