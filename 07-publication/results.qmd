...
#| fig-cap: "Plot of Shannon Index across experiment samples grouped by sampling time."
alpha_div_long %>%
...
#| fig-cap: "Log-fold changes between sample types. Circles are species enriched in the model calculus, squares are enriched in saliva, and triangles in medium. Lines are standard error. Plot shows the top 20 absolute log-fold changes between model calculus and saliva."
byoc_logf_full %>% 
  group_by(env) %>% 
  arrange(desc(abs(lfc))) %>%
  slice_head(n = 60) %>% # causes loss of some env values
  ungroup() %>%
  arrange(desc(abs(lfc))) %>%
  slice_head(n = 60) %>% # extract top 20 lfc from modern_calculus
...
```{r}
#| label: fig-genus-env
#| fig-cap: "Core microbiome of the different types of samples represented as mean relative abundances at the genus level. NA = other genera."
genus_counts_long %>%
  left_join(dna_metadata, by = c("sample" = "#SampleID")) %>%
...
#| layout-ncol: 2
#| fig-subcap:
#|  - "Spectra from day 7 samples."
#|  - "Spectra from day 12 samples."
#|  - "Spectra from day 16 samples."
#|  - "Spectra from day 24 samples (final product)."
# day 7
ftir_data %>%
  filter(
    day == 7,
