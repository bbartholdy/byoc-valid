...
ggdraw() +
    ...
    draw_plot(byoc_loadings_pc1, width = 0.70, height = 0.46, x = 0.04, y = 0.01) +
    draw_figure_label("A") +
...
We determined whether there are speices that are differentially abundant between
...
We calculated the mean relative abundances at the genus in each sample to compare
the core genera of model calculus with oral reference samples, with the most
common genera (>5% relative abundance) shown in @fig-core-genera.
...
while oral reference samples are more
diverse (@fig-core-genera), which is consistent with the
...
# align PC1 and PC2 loading plots with (0,0) on the PC1 and PC2 of the sPCA plot
ggdraw() +
    draw_plot(compar_spca_plot, width = 0.57, height = 0.44, x = 0.01, y = 0.45) +
    draw_plot(compar_loadings_pc2, width = 0.45, height = 0.48, x = 0.57, y = 0.49) +
    draw_plot(compar_loadings_pc1, width = 0.68, height = 0.45, x = 0.02, y = 0.01) +
    draw_figure_label("A") +
    draw_figure_label("B", position = "top") +
    draw_figure_label("C", position = "bottom.left")
```
