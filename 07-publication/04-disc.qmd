...
*Enterococcus* spp. are also known environmental contaminants, and we cannot
exclude environmental contamination as a possible source for these species in our
model. Early colonisers,
...
so their deficiency must be attributed to different reasons, such as nutrient
availability or being out-competed by other species.
...
[@burchamPatternsOral2020; @nearingAssessingVariation2020]. Modern and model
calculus samples are mostly separated on PC1, so this may also be caused by
the introduction of *Enterococcus* contaminants in our model.
Whether or not distinct microbial profiles, and the extracellular matrix they
produce, will affect the retention of
...
The constant disruptions in growth of *in vivo* dental plaque/calculus will contain
multiple stages of calcium phosphates, whereas our model will be more uniform.
...
Could also be causing the uniformity in crystal structure...
-->
...
exact species composition of the biofilm communities may be less important. Our
model calculus had a distinct mucrobial profile from modern reference calculus,
but ultimately resulted in a similar mineral composition.
...
For example, we know that our current methods for exploring diet and health from dental
calculus may be biased; however, with more fundamental experimentation we may also
be able to find out more about how exactly they are biased.
Using an oral biofilm model in a controlled environment with known dietary input, we
can learn more about how different methods of food processing in the past may modify the
picture we get from the samples we analyse, and how the methods we use may further distort
this picture. Our method can be used to test methods (e.g. DNA, proteomics, etc.),
decontamination protocols, as well as training on these methods and protocols without
depleting limited archaeological resources.
